...
class LARS_Finetuned(TruthMethod):
    ...
    def __init__(
        ...
        super().__init__()
        LARS_Finetuned.lars_with_context = lars_with_context
        ...
        if model_path is not None:       
            ...
            lars_tokenizer = AutoTokenizer.from_pretrained("duygunuryldz/LARS") #need to load the correct tokenizer
    ...
    def prepare_answer_text(probs, answer_tokens, edges, number_of_bins):
        ...
        for i, tkn_text in enumerate(answer_tokens):
            bin_id = LARS_Finetuned._find_bin(probs[i], edges, number_of_bins)
        ...
        if change_to_eot:
            for i, tok in enumerate(generation_token_texts):
                if tok == '</s>':
                    ...
                    changed = True
        #Test end
        a_text = LARS_Finetuned.prepare_answer_text(
            probs, generation_token_texts, self.edges, self.number_of_bins
        )
        tokenized_input = LARS_Finetuned.tokenize_input(
    ...
    @staticmethod
    def _get_datasets(
        datasets: list, size_for_each_dataset: list, val_ratio: float, seed: int, context_mode = None, dataset_csv=None, dataset_path=None, image_directory=None, retrieval_file=None
    ):
    ...
    def _generate_answers_and_label(
        ...
        **kwargs,
        ...
        for i in tqdm(range(len(train_data))):
            ...
            if LARS_Finetuned.lars_with_context:
    ...
    def generate_and_save_labeled_data(
        ...
        **kwargs,
        ...
        train_data, val_data = LARS_Finetuned._get_datasets(
            datasets,
            size_for_each_dataset,
            val_ratio,
            seed,
        ...
        train_data, val_data = LARS_Finetuned._generate_answers_and_label(
        ...
        # 5. Save if requested
        if save_data_path:
            LARS_Finetuned.save_datasets(train_data, val_data, save_data_path)
    ...
    def _prepare_data(
        ...
        for d in tqdm(train_data):
            ...
            for i in range(len(d["probs"])):
                if d["labels"][i] != -1:
                    ans_text = LARS_Finetuned.prepare_answer_text(
    ...
    def _train(
        ...
        with torch.no_grad():
            model.eval()
            tloss, metric_scores = LARS_Finetuned._test_loss(
    ...
    def train_lars_model(
        ...
        **kwargs,
        ...
        if load_data_path:
            train_data, val_data = LARS_Finetuned.load_datasets(load_data_path)
        else:
            train_data, val_data = LARS_Finetuned.generate_and_save_labeled_data(
            datasets=datasets,
        ...
        LARS_Finetuned._prepare_model_and_tokenizer(
        ...
        train_dataset, val_dataset = LARS_Finetuned._prepare_data(
            tokenizer=tokenizer,
            train_data=train_data,
            val_data=val_data,
            number_of_bins=number_of_bins,
