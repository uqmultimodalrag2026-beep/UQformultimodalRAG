...
class LeMUQ(TruthMethod):
    ...
    def __init__(
        ...
        super().__init__()
        self.name = name
        LeMUQ.lars_with_context = lars_with_context
    ...
    def prepare_answer_text(probs, answer_tokens, edges, number_of_bins):
        ...
        for i, tkn_text in enumerate(answer_tokens):
            base_bin = LeMUQ._find_bin(probs[0][i], edges, number_of_bins)
            img_no_ctx_bin = LeMUQ._find_bin(probs[1][i], edges, number_of_bins)
            no_img_ctx_bin = LeMUQ._find_bin(probs[2][i], edges, number_of_bins)
            no_img_no_ctx_bin = LeMUQ._find_bin(probs[3][i], edges, number_of_bins)

            a_text += (
                tkn_text
                + f"[prob_token_{base_bin}]"
                + f"[prob_token_img_no_ctx_{img_no_ctx_bin}]"
                + f"[prob_token_no_img_ctx_{no_img_ctx_bin}]"
                + f"[prob_token_no_img_no_ctx_{no_img_no_ctx_bin}]"
            )
        ...
        if change_to_eot:
            for i, tok in enumerate(generation_token_texts):
                if tok == '</s>':
                    ...
                    changed = True
        #Test end
        a_text = LeMUQ.prepare_answer_text(
            probs, generation_token_texts, self.edges, self.number_of_bins
        )
        #print(a_text)
        tokenized_input = LeMUQ.tokenize_input(
    ...
    def _generate_probs_conditions_llava(model, processor, all_ids, question, image, forward_hf = True):
        ...
        with torch.no_grad():
            #img_ctx
            probs = LeMUQ._infer_probs_llava(model, generated_output, tokens.shape[0])
            #img_no_ctx
            probs_img_no_ctx = LeMUQ._infer_probs_llava(model, generated_output_img_no_ctx, tokens.shape[0])
            #no_img_ctx
            probs_no_img_ctx = LeMUQ._infer_probs_llava(model, generated_output_no_img_ctx, tokens.shape[0])
            #no_img_no_ctx
            probs_no_img_no_ctx = LeMUQ._infer_probs_llava(model, generated_output_no_img_no_ctx, tokens.shape[0])
            #combine all probs
    ...
    def _generate_probs_conditions_qwen(model, processor, all_ids, input_text, question, generated_text, image, forward_hf = True):
        if forward_hf:
            ...
            question_and_context = LeMUQ.extract_between(question_text[0], "<|vision_end|>", "<|im_end|>") #everything between vision end and im_end
        ...
        probs, tokens_text = LeMUQ._infer_probs_qwen(model, processor, messages_prompt_img_context, answer_message)
        #img_no_ctx 
        probs_img_no_ctx , _ = LeMUQ._infer_probs_qwen(model, processor, messages_prompt_img_no_context, answer_message)
        #no_img_ctx 
        probs_no_img_ctx , _ = LeMUQ._infer_probs_qwen(model, processor, messages_prompt_no_img_context, answer_message)
        #no_img_no_ctx
        probs_no_img_no_ctx , _ = LeMUQ._infer_probs_qwen(model, processor, messages_prompt_no_img_no_context, answer_message)
        ...
        **kwargs,
        ...
        for i in tqdm(range(len(train_data))):
            ...
            for k in range(sampled["model_outputs"].shape[0]):
                if "Qwen" in model.config._name_or_path:
                    probs, _, full_text = LeMUQ._generate_probs_conditions_qwen(model, processor, sampled["model_outputs"][k], prompt, question, sampled['generated_texts'][k], image, forward_hf=False) #(self, model, processor, all_ids, input_text, question, generated_text, image, forward_hf = True)
                else:
                    probs, _, full_text = LeMUQ._generate_probs_conditions_llava(model, processor, sampled["model_outputs"][k], question, image, forward_hf=False)
            ...
            if "Qwen" in model.config._name_or_path:
                probs_combined_most_likely, _ , full_text= LeMUQ._generate_probs_conditions_qwen(model, processor, most_likely["model_outputs"][0][0], prompt, question, most_likely['generated_texts'][0], image, forward_hf=False)
                ...
                probs_combined_most_likely, _ , full_text= LeMUQ._generate_probs_conditions_llava(model, processor, most_likely["model_outputs"][0][0], question, image, forward_hf=False)
                train_data[i]["generated_texts"] = most_likely["generated_texts"]
                train_data[i]["probs"] = [
                    probs_combined_most_likely#.tolist()
                    #np.exp(most_likely["logprobs"][0]).tolist()
        ...
        train_data, val_data = LeMUQ.drop_negative_mismatch_generations(train_data, val_data)
        return train_data, val_data
    ...
    def generate_and_save_labeled_data(
        ...
        **kwargs,
        ...
        train_data, val_data = LeMUQ._get_datasets(
            datasets,
            size_for_each_dataset,
            val_ratio,
            seed,
        ...
        train_data, val_data = LeMUQ._generate_answers_and_label(
        ...
        # 5. Save if requested
        if save_data_path:
            LeMUQ.save_datasets(train_data, val_data, save_data_path)
        return train_data, val_data
    ...
    def _prepare_model_and_tokenizer(model, tokenizer, number_of_bins, edges):
        ...
        img_no_ctx_tokens = [f"[prob_token_img_no_ctx_{i}]" for i in range(number_of_bins)]
        no_img_ctx_tokens = [f"[prob_token_no_img_ctx_{i}]" for i in range(number_of_bins)]
        no_img_no_ctx_tokens = [f"[prob_token_no_img_no_ctx_{i}]" for i in range(number_of_bins)]
    ...
    def _prepare_data(
        ...
        for d in tqdm(train_data):
            ...
            for i in range(len(d["probs"])):
                if d["labels"][i] != -1:
                    ans_text = LeMUQ.prepare_answer_text(
    ...
    def _train(
        ...
        with torch.no_grad():
            model.eval()
            tloss, metric_scores = LeMUQ._test_loss(
    ...
    def train_lars_model(
        ...
        **kwargs,
        ...
        if load_data_path:
            train_data, val_data = LeMUQ.load_datasets(load_data_path)
        else:
            train_data, val_data = LeMUQ.generate_and_save_labeled_data(
            datasets=datasets,
        ...
        LeMUQ._prepare_model_and_tokenizer(
        ...
        train_dataset, val_dataset = LeMUQ._prepare_data(
            tokenizer=tokenizer,
            train_data=train_data,
            val_data=val_data,
            number_of_bins=number_of_bins,
